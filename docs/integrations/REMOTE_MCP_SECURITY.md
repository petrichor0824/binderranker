...
| Tool registration and result projection | BinderRanker | Three read-only tools; no host paths, raw request text, or protected operations |
...
same three path-free tools. There is no per-user workspace routing or
...
`get_current_plan`, `get_task_status`, and `inspect_dataset`. Host-side MCP
...
- exactly three read-only operations remain exposed;
