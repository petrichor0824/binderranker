...
"""
从已构建的 PlanningSession 准备一个可审核任务。

该模块是稳定的 preparation application-service seam。
它不负责自然语言理解，也不依赖模型 Provider。
...
def prepare_planning_session(
    *,
    session: PlanningSession,
    bundle_dir: Path,
    runner: WorkflowRunner | None = None,
) -> NaturalLanguagePrepareResult:
    """从已构建的 PlanningSession 准备任务。"""
    bundle_dir = bundle_dir.resolve()

    # Application boundary 的权威覆盖保护。
