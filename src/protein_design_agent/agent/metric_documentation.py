...
"""Deterministic Markdown rendering for the controlled metric ontology."""
...
def clean(value: str) -> str:
    """Normalize one Markdown cell or compact prose fragment."""
    return " ".join(value.split()).replace("|", "\\|")


def weight_text(item: MetricSemantics) -> str:
    """Render an optional direct-primary weight."""
    value = item.direct_primary_weight
    return "—" if value is None else f"{value:.2f}"


def formula_text(
    ontology: Mapping[str, MetricSemantics],
) -> str:
    """Render the direct-primary formula from one run ontology."""
    keys = (
